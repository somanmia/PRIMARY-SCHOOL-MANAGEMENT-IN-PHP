...
if(isset($_REQUEST['stuid'])){
    $id=$_REQUEST['stuid'];
    $sql="SELECT * FROM class_4 WHERE id=$id";
...
?>
    ...
    if(isset($_REQUEST['submit'])){
        ...
        if($ck==0){
            $sql="UPDATE class_4 SET Exm_year=$year,Bangla=$bangla,English=$english,Math=$math,Bangladesh_global_studies=$socail,General_science=$science, Religion=$religion WHERE id=$id";
            $run=mysqli_query($connect,$sql);
            if($run==true){
...
<section id="class_3_5_section">
   <div class="container">
      <h3 class="text-success text-center  mb-5">প্রাথমিক বিদ্যালয়ের রেজাল্ট আপডেট ক্লাস ৪ য়</h3>
